...
DB_PORT = 5432
DB_NAME = "cloud-fintech"
DB_USER = "readonly"
DB_PASS = "readonly"

WEBHOOK_URL = os.environ["WEBHOOK_URL_3"]
...
DISPOSITIONS = [
    "Agree For Partial Payments", "Agree To Pay",
    ...
    "RescheduledToNextDay", "Unacceptable Promise To Pay",
...
def format_section1(data, now_ist):
    ...
    return (
        f":bar_chart: *Fintech Cloud Dashboard — Summary Stats* | "
...
def format_section2(data, now_ist):
    ...
    # (label, quality_index or None for enqueued)
    metrics = [
        ("Enqueued",            None),
        ("Connected",           1),
        ("> 20s",               2),
    ...
    return (
        f":telephone_receiver: *Fintech Cloud Dashboard — AI Call Quality (Hourly)* | "
        f"{now_ist.strftime('%d %b %Y, %I:%M %p')} IST\n"
        f"```\n{table}\n```"
    )
...
def format_section3(data, now_ist):
    ...
    return (
        f":memo: *Fintech Cloud Dashboard — Disposition Breakdown (Hourly)* | "
        f"{now_ist.strftime('%d %b %Y, %I:%M %p')} IST\n"
        f"```\n{table}\n```"
    )
...
def main():
    ...
    print(f"Fetching Fintech Cloud dashboard data for {today_ist}...")
